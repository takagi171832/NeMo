...
def get_parser():
    ...
    parser.add_argument("--add_weight", type=List[float] , default=[0.0], help="加算用言語モデルの重み")
        ...
        for t in range(T):
            ...
            for v in top_n_idx:
                ...
                if not current_frame_index == pred_frame_index:
                    ...
                    # 加算のlmのスコアを取得する。
                    add_score = lm_add.score(" ".join((list(context) + [current_frame_char])[-2*add_ngram+1:]))*math.log(10)
                    # 減算のlmのスコアを取得する。
                    sub_score = lm_sub.score(" ".join((list(context) + [current_frame_char])[-2*sub_ngram+1:]))*math.log(10)
    ...
    for p in predicted_ids:
        if not p == pred:
            if p == len(vocab) :
                ...
                predicted_ids_list.append(vocab[int(p)])
        pred = p
    logging.info(f"predicted_ids_list: {predicted_ids_list}")
    predicted_text = "".join([i for i in predicted_ids_list if not i == "<blank>"])
    #bpeのため、最初の文字を削除する。
